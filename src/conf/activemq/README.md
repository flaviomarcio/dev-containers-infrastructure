...
Based on [openjdk:17](https://hub.docker.com/_/openjdk), as lightweight as possible AND multiarch (Support for Apple M1 chip / aarch64 aka ARM64). 

Published on the Docker Hub: https://hub.docker.com/r/${CUSTOM_ORG_NAME}/activemq

## Usage

```
docker run -it -p 61616:61616 -p 8161:8161 ${CUSTOM_ORG_NAME}/activemq:latest
```
...
${CUSTOM_ORG_NAME}/activemq:latest
```
...
```
docker tag f1aa123a520f ${CUSTOM_ORG_NAME}/activemq:5.17.4
docker push ${CUSTOM_ORG_NAME}/activemq
```
...
* `docker buildx build --push --platform linux/arm64,linux/amd64 --tag ${CUSTOM_ORG_NAME}/activemq:5.17.4 .`
* `docker buildx build --push --platform linux/arm64,linux/amd64 --tag ${CUSTOM_ORG_NAME}/activemq:latest .`
